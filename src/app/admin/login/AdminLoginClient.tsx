  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!profileData.is_admin) {
        ...
        setError('Unauthorized access. Only admin users can login here.');
      } else if (profileData.is_deleted) {
        await supabase.auth.signOut();
        setError('This admin account is deleted. Contact support.');
      } else {
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleLogin}>
          ...
          <div className="text-center">
            <Link href="/" className="text-sm text-[#006666] hover:text-[#004444]">
              Return to Home
            </Link>
          </div>
        </form>
